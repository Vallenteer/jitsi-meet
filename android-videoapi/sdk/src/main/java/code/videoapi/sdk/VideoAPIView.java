...
public class VideoAPIView extends BaseReactView<VideoAPIViewListener>
        implements OngoingConferenceTracker.OngoingConferenceListener {

    /**
     * The {@code Method}s of {@code VideoAPIViewListener} by event name i.e.
     * redux action types.
     */
    private static final Map<String, Method> LISTENER_METHODS
        = ListenerUtils.mapListenerMethods(VideoAPIViewListener.class);
    ...
    public VideoAPIView(@NonNull Context context) {
        super(context);

        // Check if the parent Activity implements VideoAPIActivityInterface,
        // otherwise things may go wrong.
        if (!(context instanceof VideoAPIActivityInterface)) {
            throw new RuntimeException("Enclosing Activity must implement VideoAPIActivityInterface");
        }
    ...
    public void enterPictureInPicture() {
        ...
        if (pipModule != null
                ...
                && !VideoAPIActivityDelegate.arePermissionsBeingRequested()
                && this.url != null) {
            try {
                pipModule.enterPictureInPicture();
            } catch (RuntimeException re) {
                VideoAPILogger.e(re, "Failed to enter PiP mode");
            }
        }
    }

    /**
     * Joins the conference specified by the given {@link VideoAPIConferenceOptions}. If there is
     ...
     *                when doing so.
     */
    public void join(@Nullable VideoAPIConferenceOptions options) {
    ...
    private void setProps(@NonNull Bundle newProps) {
        ...
        Bundle defaultProps = VideoAPI.getDefaultProps();
        Bundle url = defaultProps.getBundle("url");
        String apiID = url.getString("apiID");
        String apiKey = url.getString("apiKey");

        // Merge the default options with the newly provided ones.
        Bundle props = mergeProps(defaultProps, newProps);

        url = props.getBundle("url");
        String room = apiID + '-' + url.getString("room");
        url.putString("room", room);
        props.putBundle("url", url);
