...
const VideoLayout = {
    ...
    registerListeners() {
        ...
        document.addEventListener('videoapi-refresh-layout', () => {
            VideoLayout.refreshLayout();
        });
        document.addEventListener('videoapi-resize-layout', () => {
            VideoLayout.onResize();
        });
        document.addEventListener('videoapi-update-all-videos', () => {
            const x = document.getElementsByClassName('videocontainer__hoverOverlay');

            for (let i = 0; i < x.length; i++) {
                x.item(i).style.opacity = 0.1;
                x.item(i).style.visibility = 'visible';
            }
            setTimeout(() => {
                for (let i = 0; i < x.length; i++) {
                    x.item(i).style.opacity = 1.0;
