...
The data is now available as the identity in the VideoAPIParticipant class. You can access them by e.g. listening to the `USER_JOINED` event.
...
When JWT authentication is used with *lib-jitsi-meet* the token is passed to *VideoAPIConference* constructor:

```
var token = {token is provided by your application possibly after some authentication}

LibVideoAPI.init(initOptions).then(function(){
    connection = new LibVideoAPI.VideoAPIConnection(APP_ID, token, options);
    ...
    connection.connect();
});
