...
import * as VideoAPIConferenceEvents from '../../VideoAPIConferenceEvents';
import VideoAPIParticipant from '../../VideoAPIParticipant';
...
MockConference.prototype = Object.create(VideoAPIConference.prototype);
MockConference.prototype.constructor = VideoAPIConference;

/**
 * Mock object to be used in place of a real VideoAPIParticipant.
 *
 * @constructor
 ...
 * @returns {void}
 */
function MockVideoAPIParticipant(id, name) {
    this._jid = id;
    this._displayName = name;
}
MockVideoAPIParticipant.prototype = Object.create(VideoAPIParticipant.prototype);
MockVideoAPIParticipant.prototype.constructor = VideoAPIParticipant;
...
describe('SpeakerStatsCollector', () => {
    ...
    beforeEach(() => {
        ...
        mockConference.eventEmitter.emit(
            VideoAPIConferenceEvents.USER_JOINED,
            mockRemoteUser.id,
            new MockVideoAPIParticipant(mockRemoteUser.id, mockRemoteUser.name)
        );
    });
    ...
    describe('on user name change', () => {
        it('updates the username', () => {
            ...
            mockConference.eventEmitter.emit(
                VideoAPIConferenceEvents.DISPLAY_NAME_CHANGED,
                mockRemoteUser.id,
                newName
            );
    ...
    describe('on user leave', () => {
        it('retains the user stats but marks the user as left', () => {
            mockConference.eventEmitter.emit(
                VideoAPIConferenceEvents.USER_LEFT,
                mockRemoteUser.id
            );
    ...
    describe('on dominant speaker change', () => {
        it('updates models to reflect the new dominant speaker', () => {
            ...
            mockConference.eventEmitter.emit(
                VideoAPIConferenceEvents.DOMINANT_SPEAKER_CHANGED,
                mockRemoteUser.id
            );
