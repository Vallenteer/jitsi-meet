...
const ScreenObtainer = {
    ...
    _createObtainStreamMethod() {
        if (browser.isNWJS()) {
            return (_, onSuccess, onFailure) => {
                window.VideoAPIMeetNW.obtainDesktopStream(
                    onSuccess,
                    (error, constraints) => {
                        let jitsiError;
                        ...
                        if (error && error.name === 'InvalidStateError') {
                            jitsiError = new VideoAPITrackError(
    ...
    obtainScreenOnElectron(options = {}, onSuccess, onFailure) {
        if (window.VideoAPIMeetScreenObtainer
            && window.VideoAPIMeetScreenObtainer.openDesktopPicker) {
            ...
            window.VideoAPIMeetScreenObtainer.openDesktopPicker(
                ...
                err => onFailure(new VideoAPITrackError(
                    VideoAPITrackErrors.ELECTRON_DESKTOP_PICKER_ERROR,
                    err
                ))
            );
        } else {
            onFailure(new VideoAPITrackError(
                VideoAPITrackErrors.ELECTRON_DESKTOP_PICKER_NOT_FOUND));
        }
    },
        ...
        getDisplayMedia({
            ...
            .catch(error => {
                ...
                if (errorDetails.errorMsg && errorDetails.errorMsg.indexOf('denied by system') !== -1) {
                    ...
                    errorCallback(new VideoAPITrackError(VideoAPITrackErrors.PERMISSION_DENIED));

                    return;
                }

                errorCallback(new VideoAPITrackError(VideoAPITrackErrors.SCREENSHARING_USER_CANCELED));
            });
    },
    ...
    obtainScreenFromGetDisplayMediaRN(options, callback, errorCallback) {
        ...
        navigator.mediaDevices.getDisplayMedia({ video: true })
            ...
            .catch(() => {
                errorCallback(new VideoAPITrackError(VideoAPITrackErrors
...
function onGetStreamResponse(
    ...
    if (streamId) {
        ...
        onFailure(new VideoAPITrackError(
            VideoAPITrackErrors.SCREENSHARING_GENERIC_ERROR,
            error));
    }
}
